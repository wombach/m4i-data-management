...
def config_unflatten(arg):
    ...
    >>> config_unflatten({'foo.bar': 'val'})
    {'foo': {'bar': 'val'}}

    Nested ``list``::

    >>> config_unflatten({'foo-0': 'val', 'foo-1': 'bar'})
    {'foo': ['val', 'bar']}

    Nested ``list``\\s::

    >>> config_unflatten({'foo[0][0]': 'val'})
    {'foo': [['val']]}

    Lists of ``dict``\\s::

    >>> config_unflatten({'foo[0].bar': 'val',
    ...            'foo[1].baz': 'x'})
    {'foo': [{'bar': 'val'}, {'baz': 'x'}]}
...
_dot_or_indexes_re = re.compile(r'(__)')
...
def _parse_key(flat_key):
    ...
    parts = [split_key[0]]
    for i in range(2, len(split_key), 2):
        sep = split_key[i]
        numbers = re.findall(r'\d+', sep)
        if len(numbers) == len(sep):
            parts.extend(map(int, numbers))
        else:
            parts.append(sep)
    return parts
